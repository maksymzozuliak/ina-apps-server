...
class UserServiceMongoDBImplementation(database: MongoDatabase): UserService {
    ...
    override suspend fun updateUserInformation(userId: String, userInformation: UserInformation): Boolean {

        val user = getUserById(userId)
        return if( user != null) {
            val updateResult = userCollection.replaceOne(User::id eq user.id, user.copy(userInformation = userInformation))
            updateResult.wasAcknowledged()
//                    && updateResult.modifiedCount > 0
        } else false
    }

    override suspend fun getUserInformationById(id: String): UserInformation? {

        return userCollection.find(User::id eq id).firstOrNull()?.userInformation
    }
